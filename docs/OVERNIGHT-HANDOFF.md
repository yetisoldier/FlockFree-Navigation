...
- Branch: `master`
- Latest release work: v1.8.0 release candidate after Flock-only filtering, map cleanup defaults, and free-driving lane-widget suppression.
- Current source includes FlockFree launcher/adaptive/splash branding with red/cyan visible UI colors, the route camera summary hook, exposed FlockFree settings screen with live dynamic status refresh, Flock-only camera-data spatial indexing with source/freshness diagnostics, a nearest-Flock-camera-at-map-center inspection action with retained last-check status, OSM editor tag-prefill reporting with map-center draft action and profile-persisted report-draft status, iterative Flock camera avoidance (full block first, then progressively unblocks least-camera roads up to 4 iterations), profile-persisted applied/partial/fallback/skipped route diagnostics, movement/navigation nearby Flock camera alerts with profile-persisted last-check status plus a map-center alert test action, cache/database-backed route startup for existing Flock camera data, FlockFree map defaults that hide house/building numbers and POI labels/icons while leaving Configure Map toggles available, a lanes widget that only shows route-derived lanes during active navigation, a settings-driven CYD BLE scan/status/simulation path, CYD auto-reconnect on map resume, a CYD BLE foreground service using the Android `connectedDevice` foreground-service type with permission-gated background scan restart, phone GPS streaming to CYD over `FYGPS`, local CYD simulation from phone/OsmAnd GPS or current map center when hardware is absent, and persisted CYD detection map/review candidates.
...
- **Flock-only/map-cleanup proof, 2026-06-28 14:34 CDT:** the map-cleanup APK installed and launched on the Moto over Wi-Fi ADB at `192.168.1.139:46773`. Readiness showed `MapActivity` focused, app-code current, no FlockFree fatal launch-window entries, and the SQLite camera database populated with 89,942 Flock-only rows. Bluetooth remained off, which only affects optional CYD testing.

## Verified APK

- APK flavor: use `gplayFreeOpenglFatRelease` for public releases and `gplayFreeOpenglFatDebug` for local bench builds.
- Latest local verified artifact before the public v1.8.0 release: `build-artifacts/FlockFree-Navigation-local-20260628-map-cleanup-sideload.apk`
- Package: `com.yetiwurks.flockfree`
- Local verified SHA-256: `698ffbab622241d220048bdd64d411288b1caa78d3a065392d146201b3a7b667`
- Signature: verifies with APK Signature Scheme v2 using the FlockFree release key
- Phone install: succeeded over Wi-Fi ADB on `192.168.1.139:46773`
- Launch: succeeded into `net.osmand.plus.activities.MapActivity`; readiness confirmed populated Flock-only camera database
...
- A bundled camera source snapshot from `OsmAnd/assets/flockfree/cameras.geojson.gz` gives fresh installs an offline-first Flock camera index before the first successful network refresh.
- The current Flock-only parser filters that source snapshot to 89,942 rows and skips non-Flock records before map display, alerts, widgets, route comparison, or route avoidance can use them.
...
- Camera data now persists parsed Flock cameras to an app-private SQLite database, rebuilds a coarse in-memory spatial grid for route/helper fallbacks, and the settings screen shows camera count/bucket diagnostics when loaded.
...
- The settings screen now includes `Nearest Flock camera at map center`, which searches 5 kilometers around the current map center, opens the camera detail dialog with distance, brand/operator, direction, mount, zone, OSM ID/type, and timestamp when available, and preserves the latest found-camera or skip result in `Last nearest Flock camera`.
...
- Camera data now has Flock-filtered app-private SQLite persistence and Java fallback indexing. A full geohash/tile store is still later work.
...
8. Move the map to a suggested camera-dense anchor, tap `Nearest Flock camera at map center`, and confirm the `Nearest Flock camera` dialog opens with distance and camera metadata or `Last nearest Flock camera` preserves the found-camera/skip result.
