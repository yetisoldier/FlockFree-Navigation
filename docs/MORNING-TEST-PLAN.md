...
Current status: APK packaging should use the `gplayFreeOpenglFatDebug` flavor for local test builds and `gplayFreeOpenglFatRelease` for public release assets so the installed build includes the OpenGL renderer required for 2D/3D map mode and 3D buildings. The v1.8.0 source filters the public camera source snapshot to Flock-labeled rows, indexes 89,942 Flock cameras in SQLite, keeps map/alert/route/widget camera counts inside that Flock-only boundary, hides house/building numbers plus POI labels/icons by default, and suppresses the free-driving lane widget while preserving route-derived lane guidance during active navigation. It also retains FlockFree branding, source/freshness diagnostics, nearest-camera-at-map-center inspection, iterative relaxation camera avoidance, profile-persisted route diagnostics, nearby Flock camera alerts, OSM editor tag-prefill reporting, CYD BLE scan/status/simulation, phone GPS streaming to CYD, persisted CYD detection review candidates, US Census-backed address fallback, post-route progress reporting, active-navigation Flock camera route counts, directional vehicle arrow rendering, non-navigation road-stick tracking, and OpenGL-stable camera redraw during map animation. The latest Moto bench pass installed the OpenGL APK, launched to `MapActivity`, showed no FlockFree fatal/ANR lines, and confirmed the Flock-only camera database populated with 89,942 rows. A real drive and a camera-dense rotate/pan field pass are still recommended before calling the tracking and camera-redraw fixes fully field-proven. Run `scripts/flockfree-user-build-install.sh` before feature testing whenever app-code changes after the latest verified build.
...
- [ ] Launcher/adaptive icon, splash background, and FlockFree settings colors visibly differ from stock OsmAnd.
...
- [ ] Zoom to 10+ in an area expected to have Flock cameras.
- [ ] Flock camera dots appear on the map.
...
- [ ] Tapping a Flock camera opens the `ALPR Camera` details dialog.
...
- [ ] Move the map to a suggested camera-dense anchor, tap `Nearest Flock camera at map center`, and confirm either a `Nearest Flock camera` dialog opens or `Last nearest Flock camera` records the closest camera/skip result. A found-camera result should include distance, brand/operator, direction, and OSM metadata when available.
...
- [ ] Enable Flock camera avoidance, calculate a route, and confirm FlockFree shows a route camera summary toast with `Avoidance applied`, `Partial avoidance`, `Avoidance fallback`, or an explicit skipped reason.
...
- [ ] On an offline OsmAnd route through a known Flock camera corridor, compare the route with avoidance off versus on and look for a reroute around Flock-camera-adjacent road objects, or a partial reroute if full avoidance is not possible.
...
Pass condition: the app launches, reaches the map, does not crash, Flock camera data indexes and can be manually refreshed, nearest-Flock-camera-at-map-center inspection opens a detail dialog or preserves a `Last nearest Flock camera` result from a camera-dense anchor, Flock camera proximity alerts can be checked at the map center and preserve a `Last alert check` result, the map-center or context-menu add-camera flow pre-fills the OSM editor with ALPR tags and preserves a `Last report draft` result, the CYD settings path can connect or fail cleanly with a clear status, the status row shows recent phone GPS sends or cached GPS readiness, phone GPS reaches the CYD after connection, GPS-backed CYD detections or local phone/map-center CYD test markers become reviewable map candidates, and the experimental offline reroute visibly reports and preserves whether it applied, fell back, or skipped for a specific reason.
