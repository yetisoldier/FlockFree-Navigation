...
- [ ] Move the map to a suggested camera-dense anchor, tap `Nearest camera at map center`, and confirm either a `Nearest ALPR camera` dialog opens or `Last nearest camera` records the closest camera/skip result. A found-camera result should include distance, brand/operator, direction, and OSM metadata when available.
...
- Do not expect the camera database to be the final spatial architecture. Current source has app-private SQLite persistence plus an in-memory fallback mirror; a geohash/tile store is still later work.
...
Pass condition: the app launches, reaches the map, does not crash, camera data indexes and can be manually refreshed, nearest-camera-at-map-center inspection opens a detail dialog or preserves a `Last nearest camera` result from a camera-dense anchor, camera proximity alerts can be checked at the map center and preserve a `Last alert check` result, the map-center or context-menu add-camera flow pre-fills the OSM editor with ALPR tags and preserves a `Last report draft` result, the CYD settings path can connect or fail cleanly with a clear status, the status row shows recent phone GPS sends or cached GPS readiness, phone GPS reaches the CYD after connection, GPS-backed CYD detections or local phone/map-center CYD test markers become reviewable map candidates, and the experimental offline reroute visibly reports and preserves whether it applied, fell back, or skipped for a specific reason.
