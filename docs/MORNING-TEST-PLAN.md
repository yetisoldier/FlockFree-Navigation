...
- [ ] Background FlockFree with `CYD BLE` enabled and confirm Android shows the FlockFree CYD foreground notification. This foreground service source path is not yet validated on-device in the latest installed APK, so failures here should be logged rather than treated as a blocker for the rest of the MVP.
