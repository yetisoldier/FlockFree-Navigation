...
- Current source includes a CYD foreground service source path with a low-priority notification so BLE monitoring can stay alive when the map is backgrounded.
...
- CYD BLE integration now has a foreground service source path, but that service is not yet validated on-device in the latest installed APK. Recent detection candidates are persisted locally but not synced anywhere.
