...
- With nearby alerts enabled, FlockFree can warn while navigating or moving when the current GPS fix is within the configured alert distance of the nearest known camera. The settings screen keeps a profile-persisted `Last alert check` row so testers can see whether the last location update triggered, found no camera, hit cooldown, or skipped for accuracy/movement/data reasons.
