...
8. Enable CYD BLE, scan/connect, request CYD status, and run Simulate CYD detection; if hardware is unavailable, use the local phone-GPS test marker path.
...
cyd	TODO	Connect/simulate CYD or use the local phone-GPS simulation fallback, then confirm status, marker, or review flow.
RESULTS
}
...
write_manual_result_commands() {
  {
    ...
    printf '%s\n' "# CYD connect/status/GPS/marker/review flow observed, or local phone-GPS simulation fallback proved."
    printf 'scripts/flockfree-mark-result.py "$SESSION_DIR" cyd PASS --notes %q --summarize\n' \
      "CYD status, GPS, marker, review flow, or local simulation fallback observed"
  } > "$MANUAL_COMMANDS"
}
